...
class ModelUser():
    ...
    def register(self, db, user):
        try:
            ...
            sql = """INSERT INTO users (username, password, email, fullname, role)
                    VALUES ('{}', '{}', '{}', '{}', '{}')""".format(user.username,
                                                                    User.generate_password(user.password), user.email,
    ...
    def get_user(self, db, user_id):
        try:
            cursor = db.connection.cursor()
            # Consulta que según el token del usuario, traiga el id del usuario y según el id del usuario, traiga el
            # resto de los datos de la tabla users
            sql = """SELECT users.id, users.username, users.password, users.email, users.fullname, users.role FROM users
            ...
            if row is not None:
                # Creamos el objeto usuario
                current_user = User(row[0], row[1], row[2], row[3], row[4], row[5])
                return current_user
            else:
                return None
        except Exception as ex:
