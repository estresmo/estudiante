...
class Order:
    ...
    def add_product_to_order(self, db, user: User, product: Product, quantity: int):
        ...
        if product:
            ...
            cursor.execute("SELECT * FROM orders WHERE status = 'open' AND user_id = %s", (user.id,))
    ...
    def get_all(db, user: User):
        # user logged in
            ...
            cursor = db.connection.cursor()
            cursor.execute(
                "SELECT orders.id, orders.total, orders.created_at, orders.status, users.username FROM orders "
                "INNER JOIN users ON orders.user_id = users.id")
            orders = cursor.fetchall()
            orders_list = []
            for order in orders:
                cursor.execute(
                    "SELECT orders_detail.id,p.name,orders_detail.quantity,orders_detail.price FROM orders_detail "
                    "INNER JOIN products p on orders_detail.product_id = p.id WHERE order_id = %s", (order[0],))
                order_detail = cursor.fetchall()
                order_detail_list = []
                for od in order_detail:
